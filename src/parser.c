...
static void expr_array_resize(ExprArray *arr) {
	...
	if (arr->data == NULL)
		die_internal(
			PARSER,
			"realloc() returned null (could not allocate data for parser)"
		);
...
static void expr_array_insert(ExprArray *arr, Expr *e) {
	if (arr->len == arr->capacity) expr_array_resize(arr);
...
static Expr *expr_clone(Expr *e) {
	...
	if (e->left) lhs = expr_clone(e->left);
	if (e->right) rhs = expr_clone(e->right);
...
static void parse_expr_binary(Parser *p, ExprType type) {
	...
	else expr_array_insert(&p->state, binary(type, lhs, rhs));
